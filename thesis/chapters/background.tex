...
Whole-brain tractography results in a vast number of fibers, with the orders of magnitude typically being $10^6$ or $10^7$. To proceed with quantitative analysis, the relevant streamlines first need to be extracted from the full dataset according to anatomical criteria. The golden standard is manual placement of ROIs by an expert. The ROIs define inclusion or exclusion zones for the tracts of interest. While this method is very accurate, it has the following disadvantages: it is time-consuming, it requires an expertly trained individual, and the results are prone to inter-operator variability.
